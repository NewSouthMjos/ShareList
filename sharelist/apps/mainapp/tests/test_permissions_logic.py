...
class TestPermissionsLogicCase(TestCase):
    ...
    def test_userlist_access_check(self):
        ...
        # author rights
        self.assertEqual(userlist_access_check(user1.id, userlist1.id), 3)

        # readwrite rights
        self.assertEqual(userlist_access_check(user3.id, userlist1.id), 2)

        # readonly rights
        self.assertEqual(userlist_access_check(user2.id, userlist1.id), 1)

        # no access
        self.assertEqual(userlist_access_check(user3.id, userlist2.id), 0)

    def test_UserListCustomUser_ReadOnly_listnotfound(self):
        user = CustomUser.objects.get(username="testuser2")
        try:
            add_permission(
                userlist_id=765076763,
                user_id=user.id,
                sharelink="QGRPgaRr3-uhfjWQ",
                mode="readonly",
            )
    ...
    def test_UserListCustomUser_ReadOnly_wrong_sharelink(self):
        ...
        try:
            add_permission(
                userlist_id=userlist2.id,
                user_id=user.id,
                sharelink=wrong_sharelink,
                mode="readonly",
            )
            self.assertTrue(False)
    ...
    def test_UserListCustomUser_ReadOnly_already_exist_exception(self):
        ...
        try:
            add_permission(
                userlist_id=userlist1.id,
                user_id=user.id,
                sharelink=right_sharelink,
                mode="readonly",
            )
            self.assertTrue(False)
    ...
    def test_UserListCustomUser_ReadOnly_rightwork(self):
        ...
        try:
            UserListCustomUser_ReadOnly.objects.get(
                customuser=user.id, userlist=userlist2.id
            )
            self.assertTrue(True)
    ...
    def test_UserListCustomUser_ReadOnly_detele_permission(self):
        ...
        try:
            detele_permission(
                userlist_id=userlist1.id,
                acting_user_id=user3.id,
                user_id=user.id,
                mode="readonly",
            )
            self.assertTrue(
                False,
                msg="wrong author passed and \
    ...
    def test_set_permissions_errors(self):
        ...
        try:
            ...
            self.assertTrue(
                False, msg="Wrong author can sets permissions for list?"
            )
    ...
    def test_set_permissions_none(self):
        ...
        try:
            ...
            self.assertTrue(
                False,
                msg="Permissions didnt setted. Permissions_logic error: "
                + str(error.args),
            )
        jointable_record = UserListCustomUser_ReadOnly.objects.filter(
            customuser=user.id, userlist=userlist2.id
        )
        if len(jointable_record) >= 1:
            self.assertTrue(
                False,
                msg="Somewhy permissions are already there in R jointable (should be deleted)",
            )
        jointable_record = UserListCustomUser_ReadWrite.objects.filter(
            customuser=user.id, userlist=userlist2.id
        )
        if len(jointable_record) >= 1:
            self.assertTrue(
                False,
                msg="Somewhy permissions are already there in RW jointable (should be deleted)",
            )
    ...
    def test_set_permissions_readonly(self):
        ...
        if len(jointable_record) != 1:
            self.assertTrue(
                False, msg="Somewhy permissions are NOT there, but shoulde be"
            )
        jointable_record = UserListCustomUser_ReadWrite.objects.filter(
            customuser=user.id, userlist=userlist2.id
        )
    ...
    def test_set_permissions_readwrite(self):
        ...
        jointable_record = UserListCustomUser_ReadOnly.objects.filter(
            customuser=user, userlist=userlist2
        )
        if len(jointable_record) >= 1:
            self.assertTrue(
                False,
        ...
        if len(jointable_record) != 1:
            self.assertTrue(
                False, msg="Somewhy permissions are NOT there, but should be"
            )
