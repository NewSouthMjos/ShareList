...
class TestListItemLogicCase(TestCase):
    ...
    def test_get_userlist_detail_maininfo(self):
        ...
        try:
            ...
            self.assertTrue(
                False,
                msg="Userlist 99999999 doesnt exist, but \
        ...
        self.assertEqual(form.initial["title"], "Second")
        self.assertEqual(
    ...
    def test_get_userlist_detail_items(self):
        ...
        self.assertEqual(formset[0].initial["text"], "123")
        self.assertEqual(formset[2].initial["text"], "789")

    def test_get_userlist_detail_context(self):
        user1 = CustomUser.objects.get(username="testuser1")
        userlist2 = UserList.objects.get(title="Second")
        context = get_userlist_detail_context(user1.id, userlist2.id)
        self.assertEqual(type(context["userlist_form"]), UserListForm)
        self.assertEqual(context["item_formset"][2].initial["text"], "789")
    ...
    def test_save_userlist_detail_maininfo(self):
        ...
        request = factory.post(
            "", {"title": "POST123", "description": "TEST_DESC"}
        )
        request.user = user1
        try:
